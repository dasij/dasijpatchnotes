    ...
    <div class="main-content" :class="{ 'full-width': !showSidebar }">
      <div class="content-wrapper">
        ...
        <div class="content-header">
          <h1 class="page-title">{{ generalItem.name }} PATCH NOTES</h1>
        </div>
        ...
        <div class="content-body">
          ...
          <div v-if="!patchNotes || patchNotes.length === 0" class="empty-state">
            <div class="empty-content">
              ...
              <p class="empty-text">This section doesn't have any changes yet.</p>
              <p class="empty-subtext">Please select another item from the sidebar.</p>
            </div>
          </div>
...
export default {
  ...
  setup() {
    ...
    const loadGeneralData = async (name) => {
      try {
        const generalData = await import(`../data/general/${name}.json`);
    ...
    watch(
      ...
      (newName) => {
        ...
        loadGeneralData(normalizedName);
      },
      { immediate: true }
    );
    ...
    const onSelectItem = (payload) => {
      ...
      if (type === 'general') {
        router.push(`/general/${name.toLowerCase().replace(/ /g, '_')}`);
      } else {
