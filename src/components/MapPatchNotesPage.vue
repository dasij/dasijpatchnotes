    ...
    <div class="main-content" :class="{ 'full-width': !showSidebar }">
      <div class="content-wrapper">
        ...
        <div class="content-header">
          <h1 class="page-title">{{ map.name }} PATCH NOTES</h1>
        </div>
...
export default {
  ...
  setup() {
    ...
    const loadMapData = async (name) => {
      try {
        const mapData = await import(`../data/maps/${name}.json`);
    ...
    const onSelectItem = (payload) => {
      ...
      if (type === 'map') {
        router.push(`/map/${name.toLowerCase()}`);
      } else {
...
/* Content Wrapper - CENTRALIZADO COM ALTURA LIMITADA */
.content-wrapper {
  display: flex;
