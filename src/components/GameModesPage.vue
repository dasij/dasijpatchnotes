<template>
    <div class="bg-general-page-container bg-cover bg-no-repeat min-h-screen">
        ...
        <div class="container mx-auto py-8 relative">
            <h1 class="text-3xl font-bold mb-4 text-white">Game Modes</h1>
            <div class="general-grid">
                <div v-for="item in sortedGameModes" :key="item.id" class="text-center general-item">
                    <router-link :to="'/gamemode/' + item.name.toLowerCase().replace(/ /g, '_')" class="block relative">
...
<style scoped>
/* Copy the styles from GeneralPage.vue */
...
export default {
    ...
    methods: {
        ...
        sortGameModes() {
            this.sortedGameModes = this.gameModes.sort((a, b) => a.name.localeCompare(b.name));
        }
    },
    async created() {
